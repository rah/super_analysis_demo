...
This application has been generated by copilot-cli based on the specification contained in the file super_analysis_spec.md. No code has been modified by human intelligence :-). The code has not been reviewed in detail. User testing with a range of scenarios provides results that seem intuitive. __Do not__ use this application for financial decisions as the code has not been throughly tested and reviewed and LLMs are known to provide plausible but incorrect results.

The following text has been generated by copilot-cli.
